...
def get_produto():
    ...
    return Response(simplejson.dumps(songs_as_dict), mimetype='application/json')
...
def add_entry():
    conn = psycopg2.connect(database="cphqieql", user="cphqieql", host="jumbo.db.elephantsql.com",password="mW3yvNGp3wHFGuFp56mn4gdkdKb1UJ5T")
    cur = conn.cursor()
    cur.execute("INSERT INTO song (name, artist,album) VALUES (%s, %s, %s)", (request.form['name'], request.form['artist'], request.form['album']))
    conn.commit()
    cur.close()
    conn.close()
